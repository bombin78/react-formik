    ...
    return (
        <div className={styles.formWrap}>
            ...
            <Formik
                ...
                onSubmit={onSubmit}
                // Formik запускает проверку (валидацию):
                // 1. после любого изменения формы
                // 2. при потере фокуса после клика по любому из полей
                // 3. при нажатии кнопки отправки формы, если валидация
                //    не пройдена то обработчик не запуститься
                validationSchema={validationSchema}
                // Значение false блокирует валидацию после изменений в форме (п.1)
                // Для корректной работы формы не используем здесь значение false
                validateOnChange={true}
                // Блокирует валидацию после потери фокуса (п.2)
                                                ...
                                                phoneNumbers.map((_, idx) => (
                                                    <div key={idx}>
                                                        {/* Оставил здесь Field так как непонятно приносит ли здесь пользу компонент 
                                                          FastField или нет, количество рендеров (отрисовок) осталось без изменений */}
...
export default SimpleFormExample8;
